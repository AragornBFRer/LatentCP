...
def load_config(path: str | Path) -> ExperimentConfig:
    ...
    global_cfg = GlobalConfig(
        seeds=_listify(g_raw.get("seeds", [1])),
    ...
    em_cfg = EMConfig(
        ...
        use_X_in_em=[bool(v) for v in _listify(e_raw.get("use_X_in_em", [False]))],
    )
