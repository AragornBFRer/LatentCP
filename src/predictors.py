...
class RandomForestQuantilePredictor(Predictor):
    ...
    def fit(self, X: np.ndarray, Y: np.ndarray, **kwargs) -> "RandomForestQuantilePredictor":
        ...
        for tree in model.estimators_:
            ...
            self._leaf_targets.append(
                {leaf: np.asarray(vals, dtype=float) for leaf, vals in mapping.items()}
            )
        return self
    ...
    def predict_quantiles(self, X: np.ndarray, quantiles: Iterable[float]) -> np.ndarray:
        ...
        preds = np.zeros((X_arr.shape[0], q_arr.size), dtype=float)
        for idx, row in enumerate(X_arr):
            samples: List[np.ndarray] = []
            row_2d = row.reshape(1, -1)
            for tree, leaf_map in zip(self._model.estimators_, self._leaf_targets):
                leaf_id = int(tree.apply(row_2d)[0])
                leaf_vals = leaf_map.get(leaf_id)
            ...
            if samples:
                ...
                pool = np.array(
                    [tree.predict(row_2d)[0] for tree in self._model.estimators_], dtype=float
                )
